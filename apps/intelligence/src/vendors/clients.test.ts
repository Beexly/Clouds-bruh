...
import { allVendorClients, PrintifyClient } from './clients';
...
describe('vendor clients', () => {
  ...
  it('exposes the launch vendor client set', () => {
    expect(allVendorClients().map((client) => client.id)).toEqual(['printify', 'printful', 'cj', 'manual']);
  });
});

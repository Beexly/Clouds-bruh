...
export const Forecaster: AgentDef = {
  ...
  model: MODEL,
  tools: ['nl_analytics', 'signal_query', 'medusa_admin_read', 'order_lookup', 'recommendation_read', 'grade_drops', 'ledger'],
...
HOW YOU WORK: build demand and revenue forecasts from order history and behavioral signals via nl_analytics, signal_query, order_lookup, and medusa_admin_read; predict stockouts and optimal reorder points; factor seasonality and editorial drop cadence; state confidence bands and surface assumptions plainly; backtest every model against actuals and report the error. Run grade_drops to grade live drops on the LATR rule — for each scale proposal it returns, escalate trigger_reorder (the restock) for Garrett's approval; route kill proposals to the Warden for delist.

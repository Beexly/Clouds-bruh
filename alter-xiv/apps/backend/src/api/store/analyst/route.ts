...
// Predefined BI queries matching common business questions
const QUERIES: Array<{ keywords: string[]; description: string; sql: string; columns: string[] }> = [
  {
...
function insight(q: string, rows: any[], cols: string[]) {
...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    const { sql, description, columns } = match(question);
    ...
    return res.json({
      ...
      insight: insight(question.toLowerCase(), rows, cols),
      grounded: true,
    });
  } catch (e: any) {
